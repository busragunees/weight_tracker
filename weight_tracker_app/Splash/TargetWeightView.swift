...
struct TargetWeightView: View {
    ...
    @State var selectedPage = 50
    var data = Array(0...300).map { "\($0)" }
    
    var body: some View {
        VStack(alignment: .center){
            Spacer()
            Text("Target Weight").font(AppFont.font(type:.Bold, size: 30)).foregroundColor(AppColor.getColor(type: .DarkGray)).padding(.bottom,80)
            Text("What's your target weight ?").font(AppFont.font(type: .Light, size: 25)).foregroundColor(AppColor.getColor(type: .CardTitleColor)).padding(.horizontal,15)
           Spacer()
            ZStack {
                Ellipse()
